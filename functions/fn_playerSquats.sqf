...
if (animationState _unit == "AmovPercMstpSnonWnonDnon_exercisekneeBendB") then 
{
...
{
	[[_unit],{(_this select 0) playMove "AmovPercMstpSnonWnonDnon_exercisekneeBendB";}] remoteExecCall ["BIS_fnc_spawn", _unit, false];
	[format ["[ADMIN CONSOLE] %1 executed command: %2 perform squats", name player, name _unit]]remoteExecCall["diag_log", 2, false];
};
